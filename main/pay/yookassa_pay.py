...
def create_payment(order, cart, request):
    try:
        ...
        # Корректируем цену за единицу товара
        if total_sum > 0 and total_items_sum != total_sum:
            ...
            for item in items[:-1]:
                # Пересчитываем цену за единицу товара
                unit_price = (Decimal(item['amount']['value']) * correction_factor).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
                item['amount']['value'] = format_price(unit_price)
            ...
            # Рассчитываем корректированную цену за единицу

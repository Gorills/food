...
def format_price(price):
    formatted_price = Decimal(price).quantize(Decimal('0.00'), rounding=ROUND_HALF_UP)
    return str(formatted_price)

def adjust_prices(items, total_sum):
    """Корректировка цен, чтобы сумма товаров совпадала с total_sum"""
    total_items_sum = sum(Decimal(item['amount']['value']) * Decimal(item['quantity']) for item in items)
    ...
    if total_items_sum != total_sum:
        ...
        # Первая корректировка: пропорционально
        for item in items[:-1]:
            corrected_unit_price = (Decimal(item['amount']['value']) * correction_factor).quantize(Decimal("0.01"), rounding=ROUND_HALF_EVEN)
            item['amount']['value'] = format_price(corrected_unit_price)
            total_corrected_sum += corrected_unit_price * Decimal(item['quantity'])

        # Корректируем последний товар, чтобы компенсировать разницу
        last_item = items[-1]
        last_item_total = (total_sum - total_corrected_sum).quantize(Decimal("0.01"), rounding=ROUND_HALF_EVEN)
        corrected_last_unit_price = (last_item_total / Decimal(last_item['quantity'])).quantize(Decimal("0.01"), rounding=ROUND_HALF_EVEN)
        ...
        if final_total != total_sum:
            # Добавляем разницу к самому дешевому товару
            diff = total_sum - final_total
            cheapest_item = min(items, key=lambda x: Decimal(x['amount']['value']))
            cheapest_item['amount']['value'] = format_price(Decimal(cheapest_item['amount']['value']) + diff)
...
def create_payment(order, cart, request):
    try:
        ...
        # Корректируем цены, если суммы не совпадают
        if total_items_sum != total_sum:
            items = adjust_prices(items, total_sum)

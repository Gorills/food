...
$(document).on('keyup','#id_color_name',function(){
    var url = document.location.href;
    ...
    if(url.includes('edit')) {
      ...
      var slug = $('#data-slug').val()

      var resCol = slug + '-' + res

      if(getVal=='') {
        resCol = resCol.replace('-', '')
      }
...
$(document).on('click','.option__add',function(){
  ...
  var form_html = '<div class="option__row"><input type="text" value="' +count+ '" hidden class="optionCount"><p><label for="id_type">Тип опции:</label><select name="type" class="input" placeholder="Тип опции" id="id_type">'+ typesdss +'</select></p><p><label for="id_value">Значение:</label><input type="text" name="option_value" class="input" placeholder="Значение" maxlength="250" required="required" id="id_value"></p><p><label for="id_price">Добавить стоимость:</label><input type="number" name="option_price" class="input" placeholder="Добавить стоимость" required="required" step="0.10" id="id_price" value="0"> </p><p><label for="id_value">Вес позиции:</label><input type="text" name="option_weight" class="input" placeholder="Вес позиции" maxlength="250" required="required" id="id_option_weight"></p><p><label for="id_value">Порядок сортировки:</label><input type="text" name="option_sort" class="input" placeholder="Значение" maxlength="250" required="required" id="id_option_weightu" value="0"></p><p><label for="id_image_status">Включить изображения:</label><input type="checkbox" class="id_image_get"><input type="text" name="image_status" class="id_image_paste" value="False"></p><div class="option__minus"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path d="M416 208H32c-17.67 0-32 14.33-32 32v32c0 17.67 14.33 32 32 32h384c17.67 0 32-14.33 32-32v-32c0-17.67-14.33-32-32-32z"/></svg></div></div><div class="option__images"></div>'
